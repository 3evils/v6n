...
// -------- Action: View forum
    ...
    if ($perpage * $pages < $num)
        ++$pages;
    // ------ Build menu
    $menu1 = "<ul class='pagination'><span class='btn btn-default btn-xs'><i style='font-size: 14px;' class='fa fa-paperclip'></i>&nbsp;&nbsp;Pages&nbsp;</span> ";
    $menu2 = '';
    $lastspace = false;
    for ($i = 1; $i <= $pages; ++$i) {
        if ($i == $page)
...
";
	...
	if (!$maypost)
	{
	$HTMLOUT .="<a class='btn float-right'><i class='fa fa-check-square'></i> No Permissions</a>";
	}
	else
	{
	$HTMLOUT .="<a class='btn float-right' href='forums.php?action=newtopic&forumid=".$forumid."'><i class='fa fa-check-square'></i> Start new topic</a>";
	}
	
	// start new topic end
	$HTMLOUT .="<a class='btn float-right' href='forums.php?action=viewunread'><i class='fas fa-check fa-fw'></i> View Unread</a>
	</div>";
...
<table class='table table-bordered table-dark'>
	<thead>
		<tr>
			<th scope='col'><strong>Thread / Author</strong></th>
			<th scope='col'><strong>Rating</strong></th>
			<th scope='col'><strong>Replies</strong></th>
			<th scope='col'><strong>Views</strong></th>
			<th scope='col'><strong>Last&nbsp;post</strong></th>
		</tr>
	</thead>";
      ...
      $HTMLOUT .="<tbody>
		<tr>
				...
				<td>".get_date($topic_arr["p_added"],'DATE',1,0)."<br />by&nbsp;". $lpusername."</td>
			</tr>
				</tbody>";
		    }
	  }
