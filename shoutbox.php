...
if (isset($_GET['sent']) && ($_GET['sent'] == "yes")) {
    require_once (INCL_DIR . 'bbcode_functions.php');
    $limit = 5;
    $userid = isset($CURUSER["id"]) ? $CURUSER['id'] : '';
	$userclass = isset($CURUSER['class']) ? (int)$CURUSER['class'] : '';
    $date = TIME_NOW;
    $text = (trim($_GET["shbox_text"])) ? (trim($_GET["shbox_text"])) : '';
    $text_parsed = format_comment($text);
    ...
    if (preg_match($pattern, $text, $vars) && $CURUSER["class"] >= UC_STAFF) {
        ...
        elseif ($a[0] == $userid && (TIME_NOW - $a[1]) < $limit && $userclass < UC_STAFF) 
...
if (($shouts = $mc1->get_value('shoutbox_')) === false) {
    $res = sql_query("SELECT s.id, s.userid, s.date, s.text, s.to_user, s.staff_shout, s.autoshout, u.username, u.pirate, u.perms, u.king, u.class, u.donor, u.warned, u.leechwarn, u.avatar, u.enabled, u.chatpost FROM shoutbox AS s LEFT JOIN users AS u ON s.userid=u.id WHERE s.staff_shout ='no' ORDER BY s.id DESC LIMIT 150") or sqlerr(__FILE__, __LINE__);
...
if ($shouts && count($shouts) > 0) {
    ...
    if ($shouts) {
	    ...
	    $date = date('D g:i a');
            ...
            $HTMLOUT.= "<tr style='background-color:$bg;'><td>" . avatar_stuff($arr, 20)  . format_username($user_stuff, true) . "<span class='size1' style='color:$fontcolor;'>[$date]</span>\n$del$edit$pm$reply$private <span class='size2' style='color:$fontcolor;'>" . format_comment($arr["text"]) . "\n</span></td></tr>\n";
            

//$HTMLOUT.= "<tr style='background-color:$bg;'>" . avatar_stuff($arr, 20) . format_username($user_stuff, true) . "<span class='size1' style='color:$fontcolor;'>[$date]</span>\n$del$edit$pm$reply$private<td><span class='size2' style='color:$fontcolor;'>" . format_comment($arr["text"]) . "\n</span></td></tr>\n";
            $i++;
        } 
        $HTMLOUT.= "</table>";
    } else {
